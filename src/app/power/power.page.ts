...
const powerA_string = "110433343735"

const powerB_string = "110433353735"

const powerC_string = "110433363735"
...
export class PowerPage implements OnInit, OnDestroy {
  ...
  onTemperatureChange(buffer: ArrayBuffer) {
    ...
    if (this.resultStringArray[this.resultStringArray.length - 8] == '34' && this.resultStringArray[this.resultStringArray.length - 11] == '91') {
      if (this.resultStringArray[this.resultStringArray.length - 3] == '33' && this.resultStringArray[this.resultStringArray.length - 4] == '33' && this.resultStringArray[this.resultStringArray.length - 5] == '33') {
        ...
        this.ngZone.run(() => {
          this.power_A = this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 3], '33') +
      ...
      if (this.resultStringArray[this.resultStringArray.length - 3] == '33' && this.resultStringArray[this.resultStringArray.length - 4] == '33' && this.resultStringArray[this.resultStringArray.length - 5] == '33') {
        ...
        this.ngZone.run(() => {
          this.power_B = this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 3], '33') +
            '.' + this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 4], '33') +
            this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 5], '33');
        });
      }
      console.log('ywd:' + this.power_B);
    }
      ...
      if (this.resultStringArray[this.resultStringArray.length - 3] == '33' && this.resultStringArray[this.resultStringArray.length - 4] == '33' && this.resultStringArray[this.resultStringArray.length - 5] == '33') {
        ...
        this.ngZone.run(() => {
          this.power_C = this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 3], '33') +
            '.' + this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 4], '33') +
            this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 5], '33');
        });
      }
      console.log('ywd:' + this.power_C);
    }
    else if (this.resultStringArray[this.resultStringArray.length - 17] == '32' && this.resultStringArray[this.resultStringArray.length - 20] == '91') {

      const tempreading_A = this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 11], '33') +

        '.' + this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 12], '33') +
        this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 13], '33');

      const decimalreading_A = parseFloat(tempreading_A);
      this.listdata_A.push(decimalreading_A);

      if (this.resultStringArray[this.resultStringArray.length - 7] != '32') {
        const tempreading_B = this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 7], '33') +
          '.' + this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 8], '33') +
          this.subtractHexValue(this.resultStringArray[this.resultStringArray.length - 9], '33');
  ...
  calculateCS() {
    this.cs4la = this.sumHexValuesAndKeep8Bits((this.updatedRootaddress + powerA_string)).toUpperCase();
    this.cs4lb = this.sumHexValuesAndKeep8Bits((this.updatedRootaddress + powerB_string)).toUpperCase();
    this.cs4lc = this.sumHexValuesAndKeep8Bits((this.updatedRootaddress + powerC_string)).toUpperCase();

    this.readla = this.updatedRootaddress + powerA_string + this.cs4la + '16';
    this.readlb = this.updatedRootaddress + powerB_string + this.cs4lb + '16';
    this.readlc = this.updatedRootaddress + powerC_string + this.cs4lc + '16';

    const readlaBytes = this.hexStringToUint8Array(this.readla);
    const readlbBytes = this.hexStringToUint8Array(this.readlb);
    const readlcBytes = this.hexStringToUint8Array(this.readlc);

    this.ble.writeWithoutResponse(this.device.id, NORDIC_UART_SERVICE_UUID, NORDIC_UART_RX_CHARACTERISTIC_UUID, readlaBytes.buffer);
    this.ble.writeWithoutResponse(this.device.id, NORDIC_UART_SERVICE_UUID, NORDIC_UART_RX_CHARACTERISTIC_UUID, readlbBytes.buffer);
    this.ble.writeWithoutResponse(this.device.id, NORDIC_UART_SERVICE_UUID, NORDIC_UART_RX_CHARACTERISTIC_UUID, readlcBytes.buffer);
  }
  ...
  showcurve() {
    ...
    if (this.startDate == this.endDate) {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        this.cs4curve = this.sumHexValuesAndKeep8Bits((this.updatedRootaddress + '110A333236353433' + (33 + i * 3).toString() + this.startDate)).toUpperCase();
        console.log(this.updatedRootaddress + '110A333236353433' + (33 + i * 3).toString() + this.startDate + this.cs4curve + '16')
        this.ble.writeWithoutResponse(this.device.id, NORDIC_UART_SERVICE_UUID, NORDIC_UART_RX_CHARACTERISTIC_UUID, this.hexStringToUint8Array(this.updatedRootaddress + '110A333236353433' + (33 + i * 3).toString() + this.startDate + this.cs4curve + '16').buffer);
        this.timeout(50);
      }
    }
    else {
      for (let i = 0; i < 4; i++) {
        ...
        this.cs4curve = this.sumHexValuesAndKeep8Bits((this.updatedRootaddress + '110A333236353433' + (33 + i * 6).toString() + this.startDate)).toUpperCase();
        console.log(this.updatedRootaddress + '110A333236353433' + (33 + i * 6).toString() + this.startDate + this.cs4curve + '16')
        this.ble.writeWithoutResponse(this.device.id, NORDIC_UART_SERVICE_UUID, NORDIC_UART_RX_CHARACTERISTIC_UUID, this.hexStringToUint8Array(this.updatedRootaddress + '110A333236353433' + (33 + i * 6).toString() + this.startDate + this.cs4curve + '16').buffer);
        this.timeout(20);
      }
      for (let i = 0; i < 4; i++) {

        this.cs4curve = this.sumHexValuesAndKeep8Bits((this.updatedRootaddress + '110A333236353433' + (33 + i * 6).toString() + this.endDate)).toUpperCase();
        console.log(this.updatedRootaddress + '110A333236353433' + (33 + i * 6).toString() + this.endDate + this.cs4curve + '16')
        this.ble.writeWithoutResponse(this.device.id, NORDIC_UART_SERVICE_UUID, NORDIC_UART_RX_CHARACTERISTIC_UUID, this.hexStringToUint8Array(this.updatedRootaddress + '110A333236353433' + (33 + i * 6).toString() + this.endDate + this.cs4curve + '16').buffer);
        this.timeout(20);
      }
